...
Cleanup runs in dependency-safe order and remains bounded to the captured run graph. It covers every synthetic object the enabled groups create, including leads, lead accountability and immutable events, campaign/spend operation receipts, monthly spend, campaigns, customers, jobs, estimates and lines, inspections, documents/signatures, schedules, job child records, invoices and lines, offline payments, change orders, intake/provider logs, messages, notifications, generated office tasks, and the complete immutable automation ledger. Mighty Apes evidence uses the distinct exact prefix `TEST WTOS MIGHTY APES REGRESSION:` in both delivery and provider-lead identifiers. Before any marked source row is deleted, the Browser and dedicated Mighty Apes runners capture table-qualified sources and recursively discover every causation descendant, execution, attempt, dynamic audit, and engine-created task. The `service_role`-only `wtos_cleanup_synthetic_automation_fixture` RPC independently re-derives and locks that graph, verifies the exact pinned-project JWT claims, synthetic regression owner, company memberships, marker family, sources, IDs, and same-company causation edges, then permits the normally immutable audit-to-event delete sequence only inside its transaction and returns sanitized exact counts after zero proof. Cleanup then deletes ordinary exact captured IDs. Both harness preflight/final checks and the independent `verify-residue` process require zero automation events, executions, attempts, dynamic audits, and automation-linked tasks; the durable `rule_seeded` history remains expected.
...
### Proposal-To-Sold Job Operational Completion Extension

- Implementation commit `b694ad844af48fb23d1849f3180382a016056441` was merged as `7186001eec28177a32b454168e5fd05b43af9937` and extended the approved regression target to the exact `51/51` migration ledger. The sole additive migration is `20260824044610_native_proposal_esign_sold_job_gate.sql`, SHA-256 `703ce436ee616b5181cc189c5ea5287c64dde3f2bfaf0c57e1cc903a414e89d7`.
- Hosted and database lifecycle validation covered finalized immutable proposal revisions, company-scoped invitation exchange, active and renewed terminal signed sessions, exact signed-receipt recovery, option-bound acceptance/signature evidence, deposit-required and no-deposit conversion paths, retry/concurrency convergence to exactly one sold job, authorization/ACL isolation, rollback, and exact cleanup.
- Targeted Browser run `20260824223608414` passed its signing group and assertion across both deposit and no-deposit paths, including signed-session renewal and the exact immutable receipt. Definitive Browser run `20260824231426642` passed all `24/24` groups and `31/31` assertions with zero console errors, zero console warnings, bounded cleanup, and independently verified zero residue.
- GitHub Actions run `32790490435` passed at the exact merge SHA; repository-only job `97630910053` and protected isolated-Supabase lifecycle job `97631410575` both succeeded. CI still does not claim to execute the proprietary in-app Browser suite.
- Vercel check `52824514424` and GitHub Production deployment `6073515066` status `17277113969` succeeded for the exact merge SHA at immutable target `weathertech-31b8q060x-weathertech-os1.vercel.app`; canonical `/api/health` returned HTTP 200 with that SHA.
- Production received only the approved migration plus read-only verification. Its ledger reached `51/51`; all five new tables and the private `customer-documents` bucket remained empty, all new fields on the six existing jobs and one existing signature remained null, and all captured pre-release counts and fingerprints were unchanged. Read-only Browser smoke covered Estimates, Documents, Production Readiness, and WeatherTech/IHC isolation with zero console errors or warnings and no mutating request.
- No proposal or signature request was sent to a real customer, no real customer acceptance was created, no deposit was charged, collected, or recorded for validation, and no real sold job or other Production business record was created. The native release does not activate DocuSign or Dropbox Sign and does not weaken their separate provider gates.
