...
### AI Command Center 3.0 And Automation Rollout Preflight

- Production contains zero `ai_usage_limits` rows. The new command route requires exactly one complete, enabled policy for the selected company and fails closed before provider access when the row is missing, duplicated, disabled, or incomplete.
- WeatherTech and IHC require separate policy rows. Rows may be seeded disabled without a billing decision; company enablement and each positive monthly budget remain explicit owner decisions.
...
- `CRON_SECRET` and the authoritative input/output price-ceiling variables are not yet configured in Production. The automation scheduler and paid provider path must therefore not be described as deployed or activated.
...
- Current Production receiver: `POST https://weathertech-os.vercel.app/api/integrations/mighty-apes/yelp/webhook`. Pending canonical receiver after the reviewed rollout: `POST https://weathertech-os.vercel.app/api/integrations/mighty-apes/webhook`; the current path remains a compatibility alias.
