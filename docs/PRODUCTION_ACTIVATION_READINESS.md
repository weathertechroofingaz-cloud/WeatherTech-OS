...
### AI Command Center 3.0 And Automation Production State

- Production contains exactly two company-scoped `ai_usage_limits` rows, one for WeatherTech and one for IHC. Both are disabled and independently bounded to `openai` / `gpt-5.6-terra`, request limits of `500`, `32000` maximum request tokens, `15000` ms timeout, retry limit `1`, and a `$0` monthly budget.
- Company enablement and each positive monthly budget remain explicit owner decisions. A missing, duplicate, disabled, or incomplete policy fails closed before provider access, and one company cannot inherit the other's authorization or limit.
- The trusted quota reservation is atomic, idempotent by request, and conservative across retry-inclusive cost plus global, company, user, daily, and monthly limits.
- Only a reviewed `create_follow_up_draft` may execute, and it creates one internal office task through the centralized engine. Every other AI proposal remains reject/preview-only; no customer message, provider write, payment, or schedule mutation is authorized.
- The Production-only AI environment variables, authoritative input/output price-ceiling variables, and generated server-only `CRON_SECRET` are configured. The existing OpenAI key was preserved without reading or exposing it, and `AI_ACTION_EXECUTION_ENABLED=false` remains enforced.
- No paid Production AI smoke test has been performed or authorized by this release.

### AI And Automation Production Activation Evidence — 2026-09-02

- Production is exact at `66/66`; the fifteen-migration suffix was applied once in normal order after the `20260902024803` compatibility fix, repeat dry-run was zero, and database lint errors were zero.
- Exact redeployment `dpl_CKoXgxtMpDcRC1ekTZ3YSAxaKC5t` is `READY` at main merge `76eba068d1c08a87f09899f84f4931cd1fc07d35`; canonical `/api/health` returned HTTP 200.
- The first natural scheduler tick recorded exactly `14` `task.due` events and `14` matching audits: `10` for WeatherTech and `4` for IHC. The second and later ticks created no duplicates, executions, attempts, or new tasks.
- Provider request, AI usage, cost, business-record, and provider-record counts were unchanged. No paid AI smoke test, real provider action, or customer action was run.
- Supabase reports WAL-G backups enabled. Point-in-time recovery is not enabled and no physical backup snapshot was available, so restore granularity remains a documented residual risk rather than a completed rollback proof.
- Enabling either company policy and assigning its positive monthly budget remain owner decisions. Authoritative Mighty Apes Tucson and IHC campaign IDs, plus the applicable provider and legal gates, remain owner actions; no IDs were invented.
...
- Production applied only `20260818030913_secure_company_scoped_job_photos.sql` and `20260822054433_job_photo_storage_rollback_retry_correction.sql` for that release; at that checkpoint the linked ledger was exact at `50/50`. The subsequent `51/51` Proposal-to-Sold Job Operational Completion Phase 1 checkpoint is recorded below.
...
- Production applied only `20260824044610_native_proposal_esign_sold_job_gate.sql`, SHA-256 `703ce436ee616b5181cc189c5ea5287c64dde3f2bfaf0c57e1cc903a414e89d7`; at that release checkpoint the linked ledger was exact at `51/51`.
...
- The deployed release makes `/api/integrations/mighty-apes/webhook` canonical while retaining `/api/integrations/mighty-apes/yelp/webhook` as a compatibility alias. Its company/location campaign registry seeds and enables only the previously verified Phoenix identity; Tucson and IHC fail closed until authoritative IDs are supplied and separately enabled. `lead.test` remains audit-only, `lead.created` remains exact-once, and the legacy service correction propagates the registry-owned service/location without broadening authorization.
...
- This release is not provider-validated. The exact status is: IMPLEMENTATION/SCHEMA/DEPLOYMENT COMPLETE — OFFICIAL PROVIDER TEST AND REAL-LEAD EVIDENCE NOT YET RECORDED.
...
- Tucson voice configuration: the protected assistant destination remains present only in Vercel Production configuration and is not exposed here. It is Tucson-only, and the owner verified that it has no forwarding or ring group. Only `weathertech-tucson` is `sms_voice`. Twilio's Tucson **A call comes in** setting remains exactly `Webhook · POST https://weathertech-os.vercel.app/api/integrations/twilio/voice`; the signed SDK-generated `<Dial>` action targets `/api/integrations/twilio/voice/status`.
- Phoenix/IHC final voice decision: WeatherTech Phoenix continues ringing directly through Verizon and IHC continues ringing directly through AT&T. Neither public number forwards to Twilio. Their Twilio numbers remain exact `sms` routes with blank Voice handling and require no public-source, destination, voice-gate, or terminal-attestation variable. Obsolete Phoenix/IHC voice variables must be absent from the final Production inventory and cannot authorize a route.
...
- Compliance and outbound boundary: Twilio shows no A2P Brand or Campaign. All US long-code senders remain unregistered for outbound A2P traffic, and the shared service currently contains WeatherTech and IHC senders, so a single-business campaign must not be submitted without a separately approved company/service-separation design. Outbound SMS is hard-locked in the application and disabled in production with zero sends. MMS processing, automatic replies, reminders, campaigns, recording, transcription, and A2P outbound activation require separate approval. Any future Phoenix/IHC Twilio voice proposal would require a new explicit owner-approved rework sprint.
- Rollback and destination changes: for Tucson voice, set `TWILIO_WEATHERTECH_TUCSON_VOICE_FORWARDING_ENABLED=false` and redeploy the exact reviewed SHA before repairing its protected destination; keep Tucson's established Voice URL, SMS path, and shared incoming-message callback unchanged. Replace the Tucson destination only in protected Vercel configuration, reset its terminal attestation, redeploy, re-run masked destination/loop/readiness checks, and obtain new approval before another live call. Phoenix and IHC require no WeatherTech OS voice rollback because they remain direct carrier voice with SMS-only Twilio configuration. No rollback or destination change requires a port, release, reassignment, ownership change, or carrier modification. Keep the outbound gate false.
...
- Mighty Apes inbound credential: `MIGHTY_APES_YELP_WEBHOOK_SECRET`, configured as a Sensitive server-side Vercel variable. Current metadata includes Production and Preview scope; narrow it to Production unless an isolated Preview provider test is explicitly intended. Its value was not read or changed.
- Current canonical Production receiver: `POST https://weathertech-os.vercel.app/api/integrations/mighty-apes/webhook`. The historical `POST https://weathertech-os.vercel.app/api/integrations/mighty-apes/yelp/webhook` path remains a compatibility alias to the same handler.
...
- Safety: broad live-provider approval, public intake, public registration, customer portal, accounting writes, signature requests, and customer notifications remain disabled. The pre-existing Gmail-send and Google Calendar-write gates remain enabled and individually reported; this rollout did not broaden or exercise them.
...
WeatherTech OS is deployed, its production runtime is healthy, its evidence-proven regression contamination has been removed, and ordinary write-capable browser regression fails closed against Production Supabase. Production matches all `66/66` migrations through `20260902140838`. AI/automation deployment `dpl_CKoXgxtMpDcRC1ekTZ3YSAxaKC5t` is `READY` at exact main merge `76eba068d1c08a87f09899f84f4931cd1fc07d35`, canonical `/api/health` is HTTP 200, the scheduler's natural exact-once observation is complete, and no paid AI or real customer/provider action was run. Both company AI policies remain disabled at zero monthly budget pending separate owner decisions.

WeatherTech Final Phone Routing Reconciliation — Tucson-Only Twilio Voice is completed at implementation merge SHA `690c22083165ebd55882b51172c9ac92e1f745f8`; final configuration-cleanup deployment `dpl_ESK8ZVaXyRuqG3k2AHkfLdBuSCPi` is `READY`, canonical `/api/health` is HTTP 200 at that SHA, and the documentation-only closeout commit is identified by Git history. Its definitive frozen-tree full Browser run `20260830015820614` satisfied the unchanged completion gate: all `24/24` groups and `31/31` assertions passed, console errors/warnings were `0/0`, harness cleanup reported `residueVerified: true`, and independent `verify-residue` passed with `0` residue on the approved regression target. Proposal-to-Sold Job Operational Completion Phase 1 remains deployed from its recorded release SHA `7186001eec28177a32b454168e5fd05b43af9937`, schema-validated, and read-only Browser-validated with all five new tables empty, `customer-documents` private and empty, existing business rows unchanged, and no real proposal delivery, customer acceptance, deposit, or sold job created for validation. The native flow has signed-receipt renewal, a deposit gate, and retry-safe exactly-once sold-job conversion, but the electronic-record/customer disclosure still requires legal review before first real customer electronic-signature delivery; DocuSign and Dropbox Sign remain separately gated. Secure Company-Scoped Job Photos & Field Upload Reliability Phase 1, Lead Attribution & Marketing Accountability Phase 1, and CRM Identity Integrity Phase 1 remain within their recorded deployed boundaries.

Live Yelp Lead Intake via Mighty Apes remains implementation/schema/deployment complete but lacks official provider-test and real-lead evidence; its canonical endpoint and registry upgrade are deployed but not provider-validated. Twilio inbound SMS remains deployed: WeatherTech Tucson and WeatherTech Phoenix ending `1326` are individually live-validated, while IHC ending `6930` is exact and active at `ready_for_live_test` without live-message evidence. Tucson inbound voice forwarding is active only on the exact `sms_voice` Tucson route, with the protected assistant destination and Tucson-only POST webhook. Its two historical owner-confirmed calls completed with working two-way audio and exact bounded evidence; no new call is authorized or required by this reconciliation. WeatherTech Phoenix and IHC remain direct carrier voice lines, and their Twilio numbers remain exact SMS-only routes with blank Voice handling. No Phoenix/IHC destination, gate, terminal attestation, carrier forward, provider save, or live voice test remains outstanding. Aggregate Twilio SMS readiness remains `ready_for_live_test` solely because IHC SMS is unvalidated, and provider activation remains partial. `/api/readiness` truthfully returns HTTP 503 because broad `WTOS_PRODUCTION_APPROVED=false` while the listed provider gates remain individually reported; Tucson voice validation is not blanket Production approval. Twilio and Yelp outbound messaging remain disabled; truthful provider health, monitoring, real accountability data, and explicit owner activation decisions remain required. WAL-G backups are enabled, while PITR and a physical snapshot are absent and remain a restore-risk limitation.
