...
- Verified inbound state: the WeatherTech Tucson route, documented only by masked ending `3145`, is exact, active, and live-validated through the signature-authenticated canonical callback with two durable inbound messages and two completed provider events.
- Partial inbound state: the IHC route, documented only by masked ending `6930`, and the WeatherTech Phoenix route, documented only by masked ending `1326`, are exact and active at `ready_for_live_test`, but each has zero inbound messages and zero validation events. Phoenix is assigned to the same directly inspected shared Messaging Service, has its exact Production environment mapping, and is not yet live-validated.
