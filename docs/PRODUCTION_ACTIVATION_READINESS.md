...
- Tucson voice configuration: the protected destination remains present only in Vercel Production configuration and is not exposed here. The owner has corrected its role: the verified assistant line is Tucson-only and must not receive Phoenix or IHC calls. The latest saved Production configuration has the Tucson gate false; a Vercel environment save does not change the already-built deployment until an exact reviewed redeploy. Only `weathertech-tucson` remains `sms_voice`, while WeatherTech Phoenix plus IHC remain `sms`. Twilio's Tucson **A call comes in** setting remains exactly `Webhook · POST https://weathertech-os.vercel.app/api/integrations/twilio/voice`; the signed SDK-generated `<Dial>` action targets `/api/integrations/twilio/voice/status`. Phoenix and IHC have no voice webhook.
- Phoenix/IHC voice correction: each route requires an independently owner-chosen protected terminal destination and its own protected terminal-forwarding-disabled attestation. The current saved Production inventory has no Phoenix or IHC destination; both route enable gates and both route-specific attestations are present and explicitly `false`. The existing global terminal-attestation variable is also explicitly `false`, retains Tucson legacy meaning only, and cannot authorize Phoenix or IHC. No owner decision establishes whether Phoenix and IHC will use different terminals or an explicitly shared graph-safe sink. Those saved values are not active until an exact reviewed redeploy. Their public-source preparation did not overwrite the exact Phoenix, Tucson, or IHC Twilio ingress identity. Activation remains stopped.
...
- Rollback and destination changes: for Tucson voice, set `TWILIO_WEATHERTECH_TUCSON_VOICE_FORWARDING_ENABLED=false` and redeploy the exact reviewed SHA before repairing its protected destination; keep Tucson's established Voice URL, SMS path, and shared incoming-message callback unchanged. For an activated Phoenix/IHC route, remove carrier forwarding first, clear only that hidden ingress Voice URL, disable only that route and redeploy, then restore only its database row from `sms_voice` to `sms` if required. Replace a route destination only in protected Vercel configuration, redeploy, re-run masked destination/topology/loop/readiness checks, and obtain new approval before another live call. No rollback or destination change requires a port, release, reassignment, or ownership change. Keep the outbound gate false.
