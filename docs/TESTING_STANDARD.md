...
- `tests/ai-tools-operating-brain.test.mjs` for provider-disabled AI Tools operating-brain behavior, company scoping, role-aware financial language, prompt-safety blocking, grounded command responses, draft approval gates, and sensitive-label redaction.
- `tests/ai-tools-live-provider.test.mjs` for AI Tools 2.1 provider abstraction, disabled-provider fallback, mocked OpenAI and Anthropic calls, structured-output parsing, usage and cost controls, company-scoped retrieval, prompt-injection defense, and approval-gated action previews.
