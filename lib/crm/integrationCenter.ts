...
export const integrationProviderRegistry: IntegrationProviderMetadata[] = [
  ...
  },
  {
    ...
    shortLabel: "Yelp",
    family: "lead_intake",
    description: "Multi-account Yelp lead intake foundation with partner-required live sync, account registry routing, duplicate protection, and source attribution.",
    ...
    configurationFields: [
      {
        ...
        description: "Maps each owner-approved Yelp account to WeatherTech Phoenix, WeatherTech Tucson, or IHC.",
        required: true,
        sensitive: false,
        kind: "text",
      },
      {
        id: "partner_approval",
        label: "Yelp partner approval",
        description: "Confirms Yelp Leads API, Request-a-Quote eligibility, and business subscription access.",
        required: true,
        sensitive: false,
        kind: "text",
      },
      {
        ...
        description: "Production endpoint for partner-approved Yelp lead webhook delivery.",
        required: true,
        sensitive: false,
        kind: "webhook",
      },
      {
        ...
        description: "Server-only client credentials and redirect URI for Yelp Leads API authorization.",
        required: true,
        sensitive: true,
        kind: "secret",
      },
    ...
    credentialValidationChecks: [
      {
        ...
        description: "Validate partner webhook or controlled manual payload formats through dry-run previews before creating CRM leads.",
      },
      {
        id: "partner_access",
        label: "Partner access check",
        description: "Confirm Yelp Leads API approval, OAuth scopes, advertising eligibility, and business subscriptions.",
      },
      {
        id: "signature",
    ...
    oauthReadiness: {
      enabled: true,
      label: "OAuth and partner approval required",
      callbackPath: null,
      scopes: ["leads"],
      summary: "Official Yelp Leads API access requires Yelp-enabled OAuth scopes, business authorization, and webhook subscriptions before live sync can run.",
    },
    connectionSteps: [
      "Obtain Yelp partner approval and confirm Request-a-Quote eligibility for each business.",
      "Authorize each Yelp business account through Yelp OAuth after the app is approved for Leads API scopes.",
      "Subscribe approved business IDs to Yelp lead webhooks.",
      "Add server-side Yelp account IDs, business IDs, OAuth credentials, webhook secret, and signing secrets in hosting.",
      "Run dry-run previews for WeatherTech Phoenix, WeatherTech Tucson, and IHC.",
      "Run signed live tests before setting YELP_LIVE_SYNC_ENABLED.",
    ...
    summaryWhenDisconnected: "WeatherTech OS has the Yelp intake foundation and manual/dry-run routing, but no live Yelp account is connected or approved yet.",
  },
  {
    id: "website_forms",
