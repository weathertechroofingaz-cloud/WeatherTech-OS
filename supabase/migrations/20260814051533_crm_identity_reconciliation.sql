...
-- CRM Identity Integrity Phase 1
--
-- This migration deliberately contains no business-data backfill. Existing
-- relationships are checked before the new guards are installed, then every
-- reviewed reconciliation is completed by one authenticated, company-scoped
-- transaction.
...
create table public.crm_identity_reconciliation_events (
  ...
  constraint crm_identity_reconciliation_events_operation_key_check
    check (length(trim(operation_key)) between 8 and 180),
  constraint crm_identity_reconciliation_events_request_sha256_check
    check (request_sha256 ~ '^[0-9a-f]{64}$'),
  constraint crm_identity_reconciliation_events_decision_check
    check (decision in ('link_existing', 'create_customer', 'dismiss')),
  constraint crm_identity_reconciliation_events_decision_customer_check
    check (
      (decision = 'dismiss' and customer_id is null)
      or (decision in ('link_existing', 'create_customer') and customer_id is not null)
    ),
  constraint crm_identity_reconciliation_events_targets_object_check
    check (jsonb_typeof(selected_targets) = 'object'),
  constraint crm_identity_reconciliation_events_result_object_check
    check (jsonb_typeof(result) = 'object'),
  constraint crm_identity_reconciliation_events_company_operation_key_key
    unique (company_id, operation_key)
);

create index crm_identity_reconciliation_events_company_source_idx
on public.crm_identity_reconciliation_events (
  company_id,
  source_lead_id,
  source_updated_at,
  created_at desc
...
revoke all on table public.crm_identity_reconciliation_events from public, anon, authenticated;
grant select on table public.crm_identity_reconciliation_events to authenticated;
grant select, insert, delete on table public.crm_identity_reconciliation_events to service_role;

create or replace function public.wtos_can_reconcile_customer_property(target_company_id uuid)
returns boolean
language sql
stable
security definer
...
revoke all on function public.wtos_can_reconcile_customer_property(uuid)
from public, anon, authenticated;
grant execute on function public.wtos_can_reconcile_customer_property(uuid)
to authenticated;

create policy "CRM identity owners read reconciliation audit"
on public.crm_identity_reconciliation_events
for select
to authenticated
using (public.wtos_can_reconcile_customer_property(company_id));
...
as $$
begin
  -- The isolated regression lifecycle must remove only its exact synthetic
  -- records. Production/business audit rows remain immutable even to normal
  -- service-role application paths.
  if tg_op = 'DELETE'
    and (select auth.role()) = 'service_role'
    and old.operation_key like 'TEST WTOS REGRESSION%' then
    return old;
  end if;
...
create trigger crm_identity_reconciliation_events_immutable
before update or delete on public.crm_identity_reconciliation_events
for each row execute function public.wtos_protect_crm_identity_reconciliation_event();

create or replace function public.wtos_normalize_identity_phone(raw_value text)
returns text
language sql
immutable
strict
...
as $$
  with normalized as (
    select regexp_replace(raw_value, '[^0-9]', '', 'g') as digits
  )
  select case
...
create or replace function public.wtos_normalize_identity_email(raw_value text)
returns text
language sql
immutable
strict
set search_path = ''
as $$
  select nullif(lower(trim(raw_value)), '');
$$;

create or replace function public.wtos_normalize_identity_address(raw_value text)
returns text
language sql
immutable
strict
...
as $$
  select nullif(
    trim(regexp_replace(lower(raw_value), '[^a-z0-9]+', ' ', 'g')),
    ''
  );
$$;

revoke all on function public.wtos_normalize_identity_phone(text)
from public, anon, authenticated;
revoke all on function public.wtos_normalize_identity_email(text)
from public, anon, authenticated;
revoke all on function public.wtos_normalize_identity_address(text)
from public, anon, authenticated;

-- Abort rather than repairing any pre-existing company boundary violation.
do $$
begin
  if exists (
    select 1
...
create or replace function public.wtos_validate_crm_identity_company_links()
returns trigger
language plpgsql
security definer
set search_path = ''
...
begin
  linked_customer_id := nullif(row_data ->> 'customer_id', '')::uuid;
  linked_property_id := nullif(row_data ->> 'property_id', '')::uuid;
  linked_lead_id := nullif(row_data ->> 'lead_id', '')::uuid;
  linked_estimate_id := nullif(row_data ->> 'estimate_id', '')::uuid;
  linked_inspection_id := nullif(row_data ->> 'inspection_id', '')::uuid;
  linked_job_id := nullif(row_data ->> 'job_id', '')::uuid;
  linked_schedule_event_id := nullif(row_data ->> 'schedule_event_id', '')::uuid;

  if linked_customer_id is not null and not exists (
    select 1 from public.customers
    where id = linked_customer_id and company_id = new.company_id
  ) then
    raise exception 'Customer link must belong to the row company.' using errcode = '23514';
  end if;

  if linked_property_id is not null and not exists (
    select 1 from public.properties
    where id = linked_property_id and company_id = new.company_id
  ) then
    raise exception 'Property link must belong to the row company.' using errcode = '23514';
  end if;

  if linked_lead_id is not null and not exists (
    select 1 from public.leads
    where id = linked_lead_id and company_id = new.company_id
  ) then
    raise exception 'Lead link must belong to the row company.' using errcode = '23514';
  end if;

  if linked_estimate_id is not null and not exists (
    select 1 from public.estimates
    where id = linked_estimate_id and company_id = new.company_id
  ) then
    raise exception 'Estimate link must belong to the row company.' using errcode = '23514';
  end if;

  if linked_inspection_id is not null and not exists (
    select 1 from public.inspections
    where id = linked_inspection_id and company_id = new.company_id
  ) then
    raise exception 'Inspection link must belong to the row company.' using errcode = '23514';
  end if;

  if linked_job_id is not null and not exists (
    select 1 from public.jobs
    where id = linked_job_id and company_id = new.company_id
  ) then
    raise exception 'Job link must belong to the row company.' using errcode = '23514';
  end if;

  if linked_schedule_event_id is not null and not exists (
    select 1 from public.schedule_events
    where id = linked_schedule_event_id and company_id = new.company_id
  ) then
    raise exception 'Schedule-event link must belong to the row company.' using errcode = '23514';
  end if;

  if linked_customer_id is not null and linked_property_id is not null and not exists (
    select 1
    from public.properties
    where id = linked_property_id
      and company_id = new.company_id
      and (customer_id is null or customer_id = linked_customer_id)
  ) then
    raise exception 'Customer and property links conflict.' using errcode = '23514';
  end if;
...
revoke all on function public.wtos_validate_crm_identity_company_links()
from public, anon, authenticated;

create or replace function public.wtos_prevent_crm_identity_company_reassignment()
returns trigger
language plpgsql
security definer
set search_path = ''
...
begin
  if new.company_id is distinct from old.company_id then
    raise exception 'Company identity is immutable for reconciled CRM graph rows.'
      using errcode = '23514';
  end if;
  return new;
end;
...
revoke all on function public.wtos_prevent_crm_identity_company_reassignment()
from public, anon, authenticated;

create trigger leads_validate_crm_identity_company_links
before insert or update on public.leads
for each row execute function public.wtos_validate_crm_identity_company_links();
create trigger properties_validate_crm_identity_company_links
before insert or update on public.properties
for each row execute function public.wtos_validate_crm_identity_company_links();
create trigger estimates_validate_crm_identity_company_links
before insert or update on public.estimates
for each row execute function public.wtos_validate_crm_identity_company_links();
create trigger inspections_validate_crm_identity_company_links
before insert or update on public.inspections
for each row execute function public.wtos_validate_crm_identity_company_links();
create trigger jobs_validate_crm_identity_company_links
before insert or update on public.jobs
for each row execute function public.wtos_validate_crm_identity_company_links();
create trigger schedule_events_validate_crm_identity_company_links
before insert or update on public.schedule_events
for each row execute function public.wtos_validate_crm_identity_company_links();
create trigger office_tasks_validate_crm_identity_company_links
before insert or update on public.office_tasks
for each row execute function public.wtos_validate_crm_identity_company_links();

create trigger customers_prevent_crm_identity_company_reassignment
before update of company_id on public.customers
for each row execute function public.wtos_prevent_crm_identity_company_reassignment();
create trigger leads_prevent_crm_identity_company_reassignment
before update of company_id on public.leads
for each row execute function public.wtos_prevent_crm_identity_company_reassignment();
create trigger properties_prevent_crm_identity_company_reassignment
before update of company_id on public.properties
for each row execute function public.wtos_prevent_crm_identity_company_reassignment();
create trigger estimates_prevent_crm_identity_company_reassignment
before update of company_id on public.estimates
for each row execute function public.wtos_prevent_crm_identity_company_reassignment();
create trigger inspections_prevent_crm_identity_company_reassignment
before update of company_id on public.inspections
for each row execute function public.wtos_prevent_crm_identity_company_reassignment();
create trigger jobs_prevent_crm_identity_company_reassignment
before update of company_id on public.jobs
for each row execute function public.wtos_prevent_crm_identity_company_reassignment();
create trigger schedule_events_prevent_crm_identity_company_reassignment
before update of company_id on public.schedule_events
for each row execute function public.wtos_prevent_crm_identity_company_reassignment();
create trigger office_tasks_prevent_crm_identity_company_reassignment
before update of company_id on public.office_tasks
for each row execute function public.wtos_prevent_crm_identity_company_reassignment();
...
begin
  ...
  request_sha256 := encode(digest(reconciliation_request::text, 'sha256'), 'hex');
  ...
  normalized_name := public.wtos_normalize_identity_address(source_lead.contact_name);

  if normalized_address is null or normalized_name is null then
    raise exception 'Reconciliation lead does not contain sufficient reviewed identity.'
      using errcode = '23514';
  end if;
  ...
  for identity_lock in
    ...
    from (
      values
        ...
        ('name_address:' || normalized_name || ':' || normalized_address)
    ) as locks(lock_value)
  ...
  where customer.company_id = request_company_id
    and (
      ...
      or (
        public.wtos_normalize_identity_address(customer.property_address) = normalized_address
  ...
  if request_decision = 'link_existing' then
    ...
    evidence_types := array_remove(array[
      ...
      case when public.wtos_normalize_identity_address(target_customer.property_address) = normalized_address
        and public.wtos_normalize_identity_address(
    ...
    if trim(coalesce(reconciliation_request #>> '{customer,display_name}', source_lead.contact_name))
        <> trim(source_lead.contact_name)
      or trim(coalesce(reconciliation_request #>> '{customer,contact_name}', source_lead.contact_name))
        <> trim(source_lead.contact_name) then
  ...
  loop
    ...
    if office_task_record.lead_id is distinct from source_lead.id
      ...
      and not coalesce(office_task_record.job_id = any(job_ids), false) then
  ...
  for target in select value from jsonb_array_elements(estimate_links) loop
    ...
    linked_estimates := linked_estimates + 1;
  end loop;
  ...
  for target in select value from jsonb_array_elements(schedule_links) loop
    ...
    linked_schedules := linked_schedules + 1;
  end loop;
  ...
  for target in select value from jsonb_array_elements(inspection_links) loop
    ...
    linked_inspections := linked_inspections + 1;
  end loop;
  ...
  for target in select value from jsonb_array_elements(office_task_links) loop
    ...
    linked_office_tasks := linked_office_tasks + 1;
  end loop;
