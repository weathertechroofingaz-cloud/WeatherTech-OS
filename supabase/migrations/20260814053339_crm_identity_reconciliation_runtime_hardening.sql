...
-- Regression validation found two fail-closed runtime incompatibilities before
-- production application: pgcrypto lives outside the empty search path, and
-- the production leads table retains the legacy customer_name column. Replace
-- only the reviewed transaction boundary while preserving its locked-down
-- privileges, exact company checks, and idempotency ledger.
...
begin
  ...
  if source_lead_name is null then
    raise exception 'Reconciliation lead does not contain a usable customer name.'
      using errcode = '23514';
  end if;
