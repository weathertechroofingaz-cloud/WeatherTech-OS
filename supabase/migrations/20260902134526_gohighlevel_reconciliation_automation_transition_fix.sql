-- Forward-only reconciliation transition fix for inbound SMS and missed calls.
-- Existing migration files remain immutable; this replaces only the two trigger
-- functions and their source-table triggers.
...
begin
  if new.provider = 'gohighlevel' then
    -- Provider reconciliation updates the existing row. Suppress a replay only
    -- when OLD was already the same legitimate inbound, matched SMS shape.
    if tg_op = 'UPDATE'
      and old.provider = 'gohighlevel'
      and old.direction = 'inbound'
      and old.event_type = 'sms_inbound'
      and old.routing_status = 'matched' then
      return new;
    end if;
    ...
    if tg_op = 'UPDATE'
      and old.provider in ('twilio', 'twilio_sms')
      and old.direction = 'inbound'
      and old.event_type = 'sms_inbound'
      and old.routing_status = 'matched' then
      return new;
...
declare
  ...
  payload_fingerprint text;
begin
  -- A replay is suppressible only after the prior row was already both missed
  -- and matched. A missed + needs_review row must be allowed to emit when
  -- reconciliation updates it to matched.
  if tg_op = 'UPDATE'
    and old.call_status = 'missed'
    and old.routing_status = 'matched' then
    return new;
  end if;
